...
public interface VideoMonthlyStatsRepository extends JpaRepository<VideoMonthlyStats, Long> {
    ...
    @Query("SELECT new org.project.paysystem.dto.RankVideoInfoDto(vms.video, vms.monthlyViews, 0) "+
    ...
    @Query("SELECT new org.project.paysystem.dto.RankVideoInfoDto(vms.video, 0, vms.monthlyWatchTime) "+
            "FROM VideoMonthlyStats vms "+
            "WHERE DATE_FORMAT(vms.createdAt, '%Y-%m') = :targetMonth " +
