...
1. 프로젝트 다운로드
   ...
   Tag v1.0-2024-11-28에서 zip 또는 tar.gz 파일 다운로드 받기
...
- 구조화된 데이터 처리를 통해 데이터 일관성을 보장하며, 장애 복구와 데이터 상태 추적에 용이하도록 배치 작업을 설계
...
- **VideoDailyStats Job : 일별 통계 생성**
  ...
  - 배치 최적화를 수행하여 **데이터 처리 시간 90분 -> 6분으로 약 82% 단축**
...
위 배치 작업으로 생성된 데이터를 기반으로 통계 및 정산 조회 기능 수행
...
#### 문제점
- 데이터 규모는 증가했지만, 쿼리는 가벼운 구조라서 인덱스 적용만으로는 성능 개선 효과가 미미함

#### 캐싱 적용 이유

- 배치 작업으로 계산된 일별 정산 데이터는 자주 변경되지 않으며, 동일한 요청이 반복될 가능성이 높음

#### 적용한 전략 및 개선 효과

- Read-Aside 캐싱 전략을 적용
- Redis에 캐싱된 데이터가 있으면 DB 조회를 생략하고, 데이터를 가져옴
- DB 부하를 줄이고, 반복적인 요청 처리 속도를 단축하여 응답 속도와 시스템 성능을 최적화
...
- [모놀리식 구조 -> MSA(MicroService Architecture) 전환](https://ranny-devlog.tistory.com/entry/%EB%AA%A8%EB%86%80%EB%A6%AC%EC%8B%9D-to-MSA-%EC%A0%84%ED%99%98%EA%B8%B0-%EC%84%9C%EB%B9%84%EC%8A%A4-%EB%AA%A8%EB%93%88-%EB%B6%84%EB%A6%AC%EC%97%90-%EB%8C%80%ED%95%9C-%EC%9D%98%EC%82%AC-%EA%B2%B0%EC%A0%95)
